classdef (HandleCompatible) MeasHandler
    % MEASHANDLER Superclass for objects that return numeric values.
    % Base.MeasHandler intends to:
    %
    %       1) Standardize data structure format, while maintaining backwards compatibility.
    %       2) Provide formatted and error-checked numeric data that is easy for other programs to use.
    %       3) Guide both the user, future users, and other scripts with enforced, yet opt-in, metadata. 
    %           That is, the output will always include formatted metadata, which is autogenerated if 
    %           the user does not opt-in. 
    %
    % Base.Measurement enforces the following struct() format upon returned data:
    %
    %   d.subdata1          % Any number of fields ('subdata') which contain *formatted* and *error-checked* numeric data
    %   d.subdata2
    %   d.subdata3
    %   d....
    %   d.meta.subdata1     % Autogenerated metadata that the user can embellish based on the settings of class variables.  
    %   d.meta.subdata2
    %   d.meta.subdata3
    %   d.meta....
    %   d.meta.meta         % *Unformatted* and *unchecked* potentially-non-numeric data.
    %
    % This structure is enforced by a validation function which checks and potentially rearranges the
    % data before returning it to the user:
    %
    %   data_cleaned = Base.Measurement.validate(data_raw)
    %
...
%             obj.sizes = struct('whale', [2 2], 'orca', [1 4], 'walrus', [1 1], 'seal', [5 1], 'seaotter', [2 2 2 2 2 2 2 2 2 2]);
    ...
    methods (Sealed)
        function data = snap(obj, varargin)
            if numel(varargin) == 0     % Don't pass varargin if there isn't one.
                try
...
%                     warning(err.message)
            ...
            else     % Pass varargin if there is one.
                try
...
%                     warning(err.message)
    ...
    methods (Hidden)
        function data = measure(obj) %#ok<STOUT,MANU> % 
            error('Base.MeasHandler.measure() NotImplemented.');
		end
    end
    ...
    methods (Sealed)            % The bulk of this class. 
        ...
        function blank = blank(obj, varargin) % Gets blank data (filled with ~NaNs~ rands [debug]) corresponding to the expected structure and dimensions of the MeasHandler.
            subdata = obj.subdata;
            sizes_ = obj.getSizes();
