classdef Measurement < matlab.mixin.Heterogeneous
    ...
    %           not opt-in. 
    %
    % Base.Measurement enforces the following struct() format to outputs:
    %
    ...
    %
    % This structure is enforced by the validation function which checks and rearranges data before
    ...
    %    .data.std             : Either 1) empty or 2) a numeric array with the same size as size(d.data.dat)
    %
...
%             end
        ...
        function obj = set.sizes(obj, sizes_)
            ...
            if isstruct(obj.names) || isstruct(obj.units) || isstruct(obj.prefs) || isstruct(obj.scans)
                ...
                if isstruct(sizes_)
                    ...
                    assert(numel(new) > 0, 'Empty structs cannot contain numeric sizes, thus cannot do the job of obj.size.')
                end
                if isstruct(obj.sizes)
                    old = fieldnames(obj.sizes);
                    assert(numel(new) > 0, 'Empty structs cannot contain numeric sizes, thus cannot do the job of obj.size. obj.sizes should not have been able to set to this; something is very wrong.')
                end
                
                assert(arefieldssame(new, old));    % Make sure that the field retention happens.
            end
            ...
            if isstruct(sizes_)
                ...
                for ii = 1:numel(new)
                    assert(isnumeric(sizes_.(new{ii})) && ~isempty(sizes_.(new{ii})), 'Base.Measurement.sizes must be numeric.')
                    obj.sizes.(new{ii}) = size(squeeze(zeros(sizes_.(new{ii}))));   % See below explaination.
                end
            elseif isnumeric(sizes_) && ~isempty(sizes_)
                obj.sizes = size(squeeze(zeros(sizes_)));   % Clean up size. Added benefit of checking whether this is above the array limit, and whether this will error.
            else
                error(['Candidate "' matlab.unittest.diagnostics.ConstraintDiagnostic.getDisplayableString(sizes_) '" for Base.Measurement.sizes could not be understood.'])
            end
    ...
    methods (Hidden)
        function sizes = getSizes(obj)
            if isstruct(obj.sizes)
                sizes = obj.sizes;
            elseif isnumeric(obj.sizes) && sum(size(obj.sizes) == 1) ~= 0
                sizes.data = obj.sizes;     % Assume default subfield of .data
            else
                error()
            end
        end
        function names = getNames(obj)
            if isstruct(obj.names)
                names = obj.names;
            elseif ischar(obj.names) || isempty(obj.names)
        ...
        function scans = getScans(obj)
            if isstruct(obj.scans)
                scans = obj.scans;
            elseif isempty(obj.scans)
                scans = struct();
            else
                error()
            end
...
%                     end
    ...
    methods (Sealed)
        ...
        function d = validate(obj, raw, varargin)
...
%                     assert(~isfieldfast(raw_fields, 'std'), 'std may not be a field of d');
                    ...
                    if numel(size_fields) == 1 && ~isfieldfast(raw_fields, size_fields{1})  % If there's only one field to fill, but this field is not present.
                        ...
                        elseif isfieldfast(raw_fields, 'data')          % ...Or d.dat
...
%                             d = struct('data', raw, 'meta', []);
        ...
        function raw = validateDimension(obj, raw)
            ...
            for ii = 1:numel(subdata)
                ...
                if ~(all(~stdsize) || all(datsize == stdsize))
    ...
    methods
        ...
        function str = get_label_single(obj, whichsubdata)
			str = get_label(obj);
    ...
    methods (Hidden, Static)
        function [prefs, scans] = makeAutogeneratedPrefsScans(size_, name, varargin)
            ...
            elseif N <= 26
                ...
                error(['Insanity above maximum allowable levels: ' num2str(N) ' > 26.']);
            end
