...
if numInputs == 3
    ...
    testComObject(comObject,dlgOptions); %check comObject
    comProperties = setAllProperties(comObject,varargin{3});
    comType = class(comObject);
    comAddress = comObject.UserData;
end
end
...
switch lower(choice)
    case 'gpib'
        ...
        comType = @(x) gpib(x{1},str2double(x{2}),str2double(x{3}),varargin{3:end});
    case 'serial'
        InputArg={'comPortNum'};
    ...
    case {'tcp','tcp/ip','tcpip'}
        ...
        comType = @(x) tcpip(x{1},str2double(x{2}),varargin{3:end});
    case 'prologix'
        InputArg={'comPortNum','GPIBnum'};
        dlg_title = ['Prologix Communication Setting'];
        comType = @(x) prologix(x{1},str2double(x{2}),varargin{3:end});
    otherwise
