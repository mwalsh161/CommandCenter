function  [cent, varargout]=FastPeakFind3D(d, thres, filt ,edg, res, fid)
  ...
  %      d=d.*(d>0.9*thres);
        ...
        switch res % switch between local maxima and sub-pixel methods
            ...
            case 1 % peak find - using the local maxima approach - 1 pixel resolution
                ...
                sd=size(d);
                [I]=find(d(edg:(sd(1)-edg),edg:(sd(2)-edg),edg:(sd(3)-edg)));
                [x,y,z] = ind2sub(sd-2*edg+1,I);
                % initialize outputs
                cent=[];%
                cent_map=zeros(sd);
...
%                             (d(x(j),y(j))>d(x(j)+1,y(j))) && ...
%                             (d(x(j),y(j))>=d(x(j)+1,y(j)+1));
                    if  d(x(j),y(j),z(j)) == max(max(max(d((x(j)-1):(x(j)+1),(y(j)-1):(y(j)+1),(z(j)-1):(z(j)+1))))) &&...
                        d(x(j),y(j),z(j)) ~= min(min(min(d((x(j)-1):(x(j)+1),(y(j)-1):(y(j)+1),(z(j)-1):(z(j)+1)))))
