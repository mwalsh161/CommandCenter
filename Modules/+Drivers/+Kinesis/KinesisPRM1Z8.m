classdef KinesisPRM1Z8 < Drivers.Kinesis.Kinesis_invisible & Modules.Driver
    ...
    properties(Constant, Hidden)
        ...
        SERVOMOTORDLL='Thorlabs.MotionControl.TCube.DCServoCLI.dll';
        STEPPERMOTORCLASSNAME='Thorlabs.MotionControl.TCube.DCServo';
    end
    ...
    methods(Access=private)
        % Constructor
        function obj = KinesisPRM1Z8(serialNo, travel, name, factor)  % Instantiate the KinesisPRM1Z8 motor object
            Drivers.Kinesis.KinesisPRM1Z8.loaddlls; % Load DLLs if not already loaded 
            obj.factor = factor;
    ...
    methods(Static)
        ...
        function obj = instance(serialNo, travel, name, factor)
            mlock;
            if nargin < 2
                name = serialNo;
            end
            persistent Objects
            if isempty(Objects)
                Objects = Drivers.Kinesis.KinesisPRM1Z8.empty(1,0); % Create an empty class
            end
            ...
            obj = Drivers.Kinesis.KinesisPRM1Z8(serialNo, travel, name, factor); % Create an instance
    ...
    methods (Static)
        function loaddlls() % Load DLLs (Load all relevant dlls in case the GetDevices function was not called)
            if ~exist(Drivers.Kinesis.KinesisPRM1Z8.DEVICEMANAGERCLASSNAME,'class')
                try % Load DeviceManagerCLI dll if not already loaded
                    NET.addAssembly([Drivers.Kinesis.KinesisPRM1Z8.MOTORPATHDEFAULT,Drivers.Kinesis.KinesisPRM1Z8.DEVICEMANAGERDLL]); 
                catch
                    error('Unable to load .NET assemblies')
                end
            end
            if ~exist(Drivers.Kinesis.KinesisPRM1Z8.GENERICMOTORCLASSNAME,'class')
                try % Load in DLLs if not already loaded
                    NET.addAssembly([Drivers.Kinesis.KinesisPRM1Z8.MOTORPATHDEFAULT,Drivers.Kinesis.KinesisPRM1Z8.GENERICMOTORDLL]);
                    NET.addAssembly([Drivers.Kinesis.KinesisPRM1Z8.MOTORPATHDEFAULT,Drivers.Kinesis.KinesisPRM1Z8.SERVOMOTORDLL]);
        ...
        function motorSerialNumbers = getAvailMotors()
            ...
            for i = 1 : length(serialNumbers)
                if strcmp(serialNumbers{i}(1:2), '83')
