...
methods
    function obj = Kinesis_invisible()
        Kinesis_invisible.loadDeviceManagerdll
    end
...
methods(Static)
    function loadDeviceManagerdll() % Load DeviceManagerCLI dll
        if ~exist(Kinesis_invisible.DEVICEMANAGERCLASSNAME,'class')
            try
                NET.addAssembly([Kinesis_invisible.MOTORPATHDEFAULT,Kinesis_invisible.DEVICEMANAGERDLL]); 
            catch
    ...
    function serialNumbers=GetDevices()  % Returns a cell array of serial numbers of connected devices   
        Kinesis_invisible.loadDeviceManagerdll; % Load DeviceMnagerCLI dll if not already loaded
