classdef cwave < Modules.Driver
    ...
    % constants for C library
        Pathx64 = 'C:\Program Files (x86)\Hübner\C-WAVE Control\MatlabControl\x64\';
        Pathx86 = 'C:\Program Files (x86)\Hübner\C-WAVE Control\MatlabControl\x86\';
    ...
    methods
        ...
        function [varargout] = LibraryFunction(obj,FunctionName,varargin)
            ...
            % CWAVE DLL. Checks for error, and returns all but status
        ...
        function [status] = cwave_connect(obj)
            ...
            %Returns: int value, 1 means connection failed, 0 means successfully connected. 
        ...
        function status = set_intvalue( cmd,value)
            ...
            %% Wrightable Int Parameters are listed above in get_intvalue function comments
            %% Wrightable Wavelength stabilization parameters are listed above in get_intvalue function comments
        ...
        function status = set_floatvalue(cmd,value)
            ...
            % Returns: Returns 0 (1 before inversion) if the new value was set correctly. 
            % Returns 1 (-1 before inversion) if an error occurred.
            %% Wrightable Int Parameters are listed above in get_floatvalue function comments
            %% Wrightable Wavelength stabilization parameters are listed above in get_floatvalue function comments
        ...
        function set_target_wavelength(obj,resolution)
            %set target wavelength with either a coarse 0.01 nm resolution
            %or a fine resolution determined by wavemeter and internal PID
            %of CWAVE. 
            res_ret = ismemeber(resolution, {ob.CoarseTune; obj.FineTune});
            %res_ret = res_ret(1) || res_ret(2);
            assert(res_ret==1, 'Resolution must be either "coarse" or "fine"');
            
            if(strcmp(resolution,ob.CoarseTune))
                ...
                pause(1);
            elseif(strcmp(resolution,obj.FineTune))
                obj.WLM_PID_Compute(obj.target_wavelength);
                pause(1);
            end        
        end
