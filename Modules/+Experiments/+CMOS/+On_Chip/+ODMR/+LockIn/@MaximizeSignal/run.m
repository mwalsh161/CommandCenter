...
try
    ...
    contrastVector = NaN(1,obj.nAverages);
    for cur_nAverage = 1:obj.nAverages
        for freq = 1:obj.number_points
            ...
            if freq == obj.number_points
                
                contrastVector(cur_nAverage) =  obj.data.raw_data(2,cur_nAverage)./ obj.data.raw_data(1,cur_nAverage);
                plot(1:obj.nAverages,contrastVector,'r*--','parent',ax)
                switch obj.Mode
                    case 'voltage'
                        ylabel(ax,'Voltage (V)')
                    case 'current'
