...
%     dwell = obj.dwell_ms*1e-3; % Convert to seconds
    ...
    for i = 1 : length(obj.scan_points)
        ...
        obj.keithley.set_voltage(obj.scan_points(i)) % temporary: scan voltage applied to Keithley to scan laser wavelength
    ...
    % sweeping from current voltage to 0 volts before closing
    cur_volt = obj.keithley.get_voltage;
    for volt = linspace(cur_volt, 0, ceil(cur_volt))
