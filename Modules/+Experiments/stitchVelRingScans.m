classdef stitchVelRingScans < Modules.Experiment
    %velocity scan over a large range.
    %stitches together many piezo scans by setting the motor.
    
    properties
        dwell = 0.05;        % ms, dwell time for APDs
        scanResolution = 1e-2; %resolution for each scan.
        nSamples = 1000;      % number of samples at each point
    ...
    methods(Access=private)
        function obj = stitchVelRingScans()
            ...
            obj.trackerEx.object_size = 0.015;
            obj.trackerCo.object_size = 0.04;    
        end
    end
    ...
    methods(Static)
        function obj = instance()
            ...
            if isempty(Object) || ~isvalid(Object)
                Object = Experiments.stitchVelRingScans();
            end
            obj = Object;
        end
    end
    ...
    methods
        ...
        function runSlow(obj,statusH,ax)
            ...
            for ii=1:1:3
                    try
                        obj.trackerEx.Track(true);
                        
                    catch err
                        msg = sprintf('with privious Position, Proceed? (1: yes, 2: no)');
                        
                        answer = inputdlg(msg,'Notice!');
 
                        if str2num(cell2mat(answer))==2
                            rethrow(err);
                        end
                    end

                    try
                        obj.trackerCo.Track(true);
            ...
            for i = 1:nCoarse
                ...
                max_count = max(obj.data(2,:)./obj.data(3,:));
                
                if (obj.data(2,d)/obj.data(3,d))>0.35*max_count
                    for jj=1:1:2
                        try
                            obj.trackerEx.Track(true);
