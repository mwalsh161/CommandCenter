classdef QR < Modules.Imaging
    ...
    properties(Constant, Hidden)
        displaymodes = {'Raw', 'Flattened', 'Convolution X', 'Convolution Y', '(Convolution X)^3 + (Convolution Y)^3', 'Thresholded'};
        displaylevels = {'All candidates', 'Valid checksum only', 'Self-consistent only'};
    end
    ...
    properties(GetObservable,SetObservable)
        ...
        rotate = Prefs.MultipleChoice(0, 'set', 'set_rotate', 'allow_empty', true, 'choices', {0, 90, 180, 270}, ...
        ...
        displaymode =  Prefs.MultipleChoice(Imaging.QR.displaymodes{1},  'set', 'set_variable', 'allow_empty', false, 'choices', Imaging.QR.displaymodes, ...
        ...
        displaylevel = Prefs.MultipleChoice(Imaging.QR.displaylevels{1}, 'set', 'set_variable', 'allow_empty', false, 'choices', Imaging.QR.displaylevels, ...
                                                                        'help_text', 'These displaylevels allow the user to hide unnecessary information such as QRs which do not decode.');
        ...
        N = Prefs.Integer(0,    'readonly', true,                       'help_text', 'Number of self-consistent QR codes within a field of view.')
    end
    
    % Variables required for imaging modules. This should be cleaned up in the future.
    properties
        maxROI = [-1 1; -1 1];
        prefs = {'image', 'flip', 'rotate', 'displaymode', 'displaylevel', 'calibration', 'QR_ang'};
    end
    properties(GetObservable,SetObservable)
        resolution = [120 120];
        ROI = [-1 1;-1 1];
    ...
    methods
        ...
        function val = set_rotate(obj, val, ~)
            if val == 90 || val == 270
                obj.image.maxROI
                obj.maxROI = obj.image.maxROI([2, 1], :);
            else
                obj.maxROI = obj.image.maxROI;
            end
            obj.ROI = obj.maxROI;
            obj.analyze();
        end
    ...
    methods
        ...
        function displayimg = analyze(obj)
            ...
            if obj.N >= 2
                ...
                if abs(options_fit.calibration / obj.calibration - 1) < .05
            ...
            switch obj.displaymode
                case Imaging.QR.displaymodes{1}
                    displayimg = img;
                case Imaging.QR.displaymodes{2}
                    displayimg = stages.flat;
                case Imaging.QR.displaymodes{3}
                    displayimg = stages.convH;
                case Imaging.QR.displaymodes{4}
                    displayimg = stages.convV;
                case Imaging.QR.displaymodes{5}
                    displayimg = stages.conv;
                case Imaging.QR.displaymodes{6}
            ...
            cx = obj.calibration * (obj.ROI(1,1) + obj.ROI(1,2))/2;
            cy = obj.calibration * (obj.ROI(2,1) + obj.ROI(2,2))/2;
            ...
            obj.graphics.center.XData = cx;
            obj.graphics.center.YData = cy;
            obj.graphics.centertext.Position = [cx, cy, 0];
            ...
            for ii = 1:size(v,2)
                ...
                if isnan(V(1,ii))
                    ...
                    switch obj.displaylevel
                        case Imaging.QR.displaylevels{3}
                            shoulddisplay = ~options_fit.outliers(ii);
                    end
            ...
            switch obj.displaylevel
                case Imaging.QR.displaylevels{1}
                    obj.graphics.p2.Visible = true;
                    obj.graphics.p3.Visible = true;
                case Imaging.QR.displaylevels{2}
                    obj.graphics.p2.Visible = true;
                    obj.graphics.p3.Visible = false;
                case Imaging.QR.displaylevels{3}
                    obj.graphics.p2.Visible = false;
                    obj.graphics.p3.Visible = false;
            end
            
            gdata = (affine(floor(options_fit.Vcen) + [[0, 1, 1, 0, 0]; [0, 0, 1, 1, 0]], options_fit.M,  options_fit.b)  - obj.ROI(:,1) ) * options_fit.calibration;
            gdata = [gdata [NaN; NaN], (affine(floor(options_fit.Vcen) + [[0, 1, 0, 1, 0]; [0, 0, 1, 1, 0]], options_fit.M, options_fit.b) - obj.ROI(:,1) ) * options_fit.calibration];
