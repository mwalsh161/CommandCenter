classdef Msquared < Modules.Source & Sources.TunableLaser_invisible
    ...
    properties(SetObservable,GetObservable)
        ...
        refresh =           Prefs.Button('units', 'Poll hwserver',   'set', 'set_refresh', ... 'get', 'get_refresh', 
                                                                    'help_text', 'Get information (voltages, wavelengths, states) from the laser and wavemeters');
        abort =             Prefs.Button('units', 'Stop Tuning',     'set', 'set_abort', ...
                                                                    'help_text', 'Stop the current tuning operation.');
        
        % WAVELENGTH prefs
        setpoint_ =         Prefs.Double(NaN,   'units', 'nm',  'set', 'set_target_wavelength', ...
                                                                    'help_text', ['Use this knob to tune the laser to a certain wavelength. ' newline ...
                                                                    'Availible ranges: ECD-X (350-525), EMM (515-582/580-661), SolsTiS (700-1100). ' newline ...
                                                                    'The laser will decide the appropriate module to use based on the chosen wavelength']);
        center_percent =    Prefs.Double(100,   'units', '%', ...
                                                                    'help_text', ['Acceptable range for the resonator percent to lie within about 50%. ' newline  ...
                                                                    '100% is the full range (disables this feature). If the laser is outside this range ' newline ...
                                                                    'after tuning, further attempts will be made by repeatedly moving to and from the target ' newline ...
                                                                    'wavelength until a good resonator percent value is found. The laser automatically ' newline ...
                                                                    'targets 50%, but will stray if the etalon setpoint is unfavorable.']);
