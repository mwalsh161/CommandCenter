...
%res_laser.on;
%res_laser.set_target_wavelength(619.29);
%%
...
Blue_sweep = linspace(10, 50, 10); % filter wheel angle
Green_sweep = linspace(5, 15, 5); % mW
Res_hwp_sweep = linspace(86, 136, 10); % half wave plate angle
%%
% Setting Counter
apd1.dwell = 0.01;
...
apd1.WindowMax = 10;
%%
% for b = 1 : length(Blue_sweep)
%     for r = 1 : length(Res_hwp_sweep)
%         Counts(b, r). xData = NaN(1, ceil(apd1.WindowMax/apd1.update_rate));
%     end
% end
...
%         apd1.start;
%         apd1.stop;
        Counts_blue(b, r).yData = double(apd1.singleShot(apd1.dwell, 50000000));
%         Counts(b, r).xData = apd1.pltH.XData;
%         Counts(b, r).yData = apd1.pltH.YData;
        figure()
...
% function hwp_pos = HWP_Rot(angle) 
%     hwp.move(angle);
%     while hwp.Moving
%         pause(0.1)
%     end
%     hwp_pos = hwp.Position;
% end
%%
for i = 1 : 297
    saveas(figure(i), strcat('C:\Users\Experiment\Documents\Cassie Local\Blue', num2str(i), '.jpg'))
    close(figure(i))
end