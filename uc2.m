function uc2(ms, managers)
    folder = 'Z:\Experiments\Diamond\UC2\2021_09_10 RoMi\';
...
%     N = (20-5+1)*89-13;
    ...
    for ii = 1:length(xsweep)
        for jj = 1:length(ysweep)
            ...
            if n > N && ~( (dX < minx || dX > maxx) && (dY < minx || dY > maxx) )
                if ~mod(ii, 2)
                    dY = ysweep(end+1-jj);
                end
...
%         angles = [0 0 120 180 0];
        names = {'wl', 'g_550LP_633LP', 'g_550LP_SiVZPL', 'g_550LP_GeVZPL', 'g_550LP_SnVZPL', 'wlpost'};
